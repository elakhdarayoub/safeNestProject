...
export default function EditArticlePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!res.ok) {
        setLoadError('Article not found or could not be loaded.')
        setIsLoading(false)
        return
      }
  ...
  const onSubmit = async (data: ArticleFormData) => {
    ...
    if (json.success) {
      ...
      setServerError(json.error ?? 'Failed to update article. Please try again.');
    }
  };

  if (isLoading) {
    return (
      <main className="pt-[140px] pb-[128px] flex items-center justify-center">
  ...
  return (
    <main className="pt-[140px] pb-[128px]">
      <section className="max-w-[800px] mx-auto px-8">
        ...
        <h1 className="font-display text-[40px] font-bold leading-[1.05] text-white mb-2">Edit Article</h1>
        <p className="text-[16px] text-content-secondary mb-10">Update the content of an existing article on the SafeNest platform.</p>
